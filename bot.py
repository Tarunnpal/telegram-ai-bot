...
def _call_gemini_with_full_context(chat_id: int, first_name: str, prompt: str) -> str:
    ...
    system_instruction = (
        ...
        f"- ROUTE & TRAIN SCHEDULE GUIDANCE (CRITICAL):\n"
        f"  * When asked for trains between Source and Destination (e.g., 'Delhi to Mumbai trains', 'Jaipur to Bangalore train timetable'):\n"
        f"  * List prominent trains running on that route with Train Number, Train Name, Departure Time, Arrival Time, Days of Running, and Travel Duration.\n"
        f"  * Format the train timetable cleanly with bullet points or structured lists for easy reading.\n"
        f"  * Provide accurate IRCTC cancellation & refund rules (Confirm/RAC/Waitlist charges, TDR filing).\n"
        f"  * Advise users to verify real-time seat availability & live booking on official IRCTC (irctc.co.in) or NTES / WhereIsMyTrain app.\n"
