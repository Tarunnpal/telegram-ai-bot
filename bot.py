...
def _call_gemini_with_full_context(chat_id: int, first_name: str, prompt: str) -> str:
    ...
    system_instruction = (
        ...
        f"- ROUTE, TRAIN SCHEDULE & LIVE LOCATION GUIDANCE (EXACT DIALOGUE FLOW):\n"
        f"  * TRAIN NUMBER & TIMETABLE LOOKUP: When asked about a train number (e.g., '12952', '12004') or route trains:\n"
        f"    - Provide Train Name, Route (Source -> Destination), Departure/Arrival Timings, Days of Running, and Classes.\n"
        f"  * LIVE TRAIN RUNNING STATUS / LOCATION QUERY:\n"
        f"    - When asked for live location/running status of a train (e.g., 'Train 12952 live location', 'Where is train 12951 right now?'):\n"
        f"    - Provide expected current station breakdown based on live time ({date_info}), and provide direct 1-tap Live GPS Tracking link: https://www.google.com/search?q=live+running+status+train+TRAINNUMBER and NTES / WhereIsMyTrain guidance.\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "3. <b>Persistent Memory 🧠</b>: Your chat context and past conversations are permanently saved across sessions."
    )
    if update.message:
