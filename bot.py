...
def _call_gemini_with_full_context(chat_id: int, first_name: str, prompt: str) -> str:
    ...
    system_instruction = (
        ...
        f"  * When asked for trains between Source and Destination, timetables, train numbers, or train cancellation/booking info:\n"
        ...
        f"  * CRITICAL PROACTIVE TICKET BOOKING OFFER: At the end of EVERY train-related query response, ALWAYS ask politely:\n"
        f"    '🎟️ Kya main aapki train ticket book karne ki process ko aage badhau?'\n"
        f"  * If the user says 'Haan' or 'Yes' to ticket booking: Ask for their journey date, passenger details, class choice (1A/2A/3A/SL), and guide them with direct link to official IRCTC (irctc.co.in) booking!\n"
